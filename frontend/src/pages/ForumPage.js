...
import EditTopicModal from '../components/modals/EditTopicModal'; // Import the modal
...
const ForumPage = () => {
  ...
  const isEditModalOpen = !!editingTopic;
  ...
  const handleTopicUpdated = (updatedTopic) => {
    // Update the topic in the local state to see changes immediately
    setForum(prevForum => ({
      ...prevForum,
      topics: prevForum.topics.map(t => t.id === updatedTopic.id ? updatedTopic : t)
    }));
  };
  ...
  const handleDeleteTopic = async (topicSlug) => {
    if (window.confirm('Are you sure you want to delete this topic? This will also delete all its posts and cannot be undone.')) {
        try {
            await apiService.deleteTopic(topicSlug);
            // Re-fetch forum data to show the updated topic list and counts
            loadForumDetails();
        } catch (err) {
                ...
                <div className="topic-title-container">
                  ...
                  {isAuthenticated && (user?.isAdmin || user?.userId === topic.user?.id) && (
                    <div className="topic-actions">
                      <button onClick={() => handleOpenEditModal(topic)} className="btn-sm btn-secondary">Edit</button>
                      <button onClick={() => handleDeleteTopic(topic.slug)} className="btn-sm btn-danger">Delete</button>
                    </div>
                  )}
                </div>
                <small>
